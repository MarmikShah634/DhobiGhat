...
export interface OtpVerifyResponse extends AuthTokens {
  role: string; is_new_user: boolean; temp_token?: string;
}

export const authApi = {
  requestOtp: (phone: string) =>
    apiClient.post('/auth/otp/request', { phone, role: 'washerman' }),
  verifyOtp: (phone: string, otp: string) =>
    apiClient.post<OtpVerifyResponse>('/auth/otp/verify', { phone, otp, role: 'washerman' }),

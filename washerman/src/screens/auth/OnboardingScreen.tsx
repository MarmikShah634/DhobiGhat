...
const PAGES = [
  { icon: '📦', title: 'Manage All Your Orders in One Place', subtitle: 'Accept, track and deliver — all from your phone.' },
  { icon: '🔔', title: 'Get Notified Instantly for New Orders', subtitle: 'Never miss a customer with real-time push alerts.' },
  { icon: '📊', title: 'Track Your Earnings and Send Receipts', subtitle: 'Professional invoices and ledger at your fingertips.' },
];
  ...
  return (
    <LinearGradient colors={['#050D1F', '#0F1F3D']} style={styles.container}>
      ...
      <View style={styles.dots}>
        {PAGES.map((_, i) => <View key={i} style={[styles.dot, i === current && styles.dotActive]} />)}
      </View>
      <View style={styles.footer}>
        <Button label={current === PAGES.length - 1 ? 'Get Started' : 'Next'} onPress={goNext} />
      </View>
    </LinearGradient>
  );
}

...
const schema = z.object({ name: z.string().min(2), business_name: z.string().min(2), area: z.string().min(2), address: z.string().optional() });
...
export function EditProfilePage() {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<FormData>({
    ...
    defaultValues: { name: washerman?.name ?? '', business_name: washerman?.business_name ?? '', area: washerman?.area ?? '', address: washerman?.address ?? '' },
  });

  const onSubmit = async (data: FormData) => {
    try {
      const res = await washermenApi.updateProfile(data);
